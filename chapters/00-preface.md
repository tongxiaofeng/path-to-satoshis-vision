...
人类几千年的经济活动——从最早的贝壳交易到今天的黄金结算——都建立在这个前提之上。我们之所以能用"钱"来交换物品，最底层的原因是：钱花出去就没有了。一笔钱不能同时花两次。
...
信息是一种抽象的模式。一段文字、一首歌、一张图片——它们的本质是一串比特，一组0和1的排列。和物理世界中的原子不同，比特可以被精确、无损、零成本地复制。你复制一个文件，原件不会少一个字节。比特和它的载体天然可分离——同一串数据可以同时存在于一万块硬盘上，每一份都是完美的原件。
...
数字签名——密码学中的公私钥技术——可以证明"这笔转账确实是张三授权的"。但它不能证明"张三没有用同一笔钱给别人也转过一次"。签名解决的是身份和授权问题，而双花是一个完全不同的问题：同一笔钱是否已经被花过？

这就是数字货币的原罪。在解决双花之前，数字世界里不可能存在真正的"钱"。
...
这不是理论上的风险。2013年，塞浦路斯政府在金融危机中直接从储户账户中划走存款来救助银行。2022年，加拿大政府冻结了参与"自由车队"抗议的卡车司机的银行账户——不经法院审判，直接冻结。在这些事件中，人们突然意识到一个残酷的事实：存在银行里的钱，在某种意义上并不完全是"你的"。它的使用需要银行的配合，而银行的配合需要政府的许可。
...
"Works well enough"——"足够好"。这两个字里藏着一个巨大的妥协。大多数时候，银行确实不会对你搞鬼。但"大多数时候"不是"永远"。每一次金融危机、每一次资本管制、每一次账户冻结，都在提醒我们：基于信任的系统，其安全上限就是你信任对象的可靠程度。
...
因为银行要承担调解纠纷的责任，它必须对每笔交易收取足够高的手续费来覆盖这个成本。这意味着小额支付在经济上变得不划算——给街头艺人打赏一块钱，手续费可能比打赏金额还高。信任的代价不仅是抽象的风险，还是实实在在的经济摩擦。
...
1983年，密码学家David Chaum提出了盲签名技术，奠定了匿名数字支付的理论基础。1989年，他创立了DigiCash——第一个实际运行的匿名数字支付系统。DigiCash在密码学上是优雅的，但它运行的前提是有一家银行愿意作为可信第三方来验证交易、防止双花。当唯一合作的银行退出后，DigiCash于1998年破产。技术再优雅，也抵不过中心化的单点脆弱。

1997年，Adam Back提出了Hashcash，用计算难题来对抗垃圾邮件——这是工作量证明概念的先驱。1998年，Wei Dai发表了b-money提案，第一次系统性地描述了去中心化数字货币的构想，但缺少可行的共识机制。同年，Nick Szabo提出了Bit Gold方案，结合了工作量证明和时间戳链，是所有前比特币设计中最接近终点的一个，但始终未能解决去中心化环境下的共识收敛问题。2004年，Hal Finney基于Hashcash构建了RPOW——可复用的工作量证明系统，但验证仍然依赖一台可信服务器。
...
这不是一种可选的解决方案，也不是诸多方案中的一种。这是唯一的机制。
...
问题于是变得尖锐了：排序需要一个排序者。银行充当排序者，代价是你必须信任银行。我们不想信任任何人。那么——**谁来排序？**
...
他的思路可以用一句大白话来概括：**如果不能信任任何一个人，那就让所有人去竞争，谁付出的代价最大，谁来排序。**
...
"Distributed timestamp server"——分布式时间戳服务器。比特币不是一个"加密货币系统"，不是一个"去中心化数据库"。在中本聪的定义中，它是一台给交易盖时间戳的机器。时间戳的功能是什么？确定先后顺序。先后顺序解决什么问题？双花。
...
第三步，第一个猜对的矿工赢得这一轮竞争。他把自己的那一页"账本草稿"广播给全网，其他矿工验证答案是否正确——这个验证过程极快，瞬间完成。验证通过后，这一页就成为正式的账本记录——一个区块。

第四步，所有矿工在这个新区块的基础上，开始下一轮竞争。

这个过程大约每十分钟发生一次。网络会自动调整数学题的难度，确保不管全球有多少矿工参与竞争，平均出块时间始终维持在十分钟左右。
...
这段话的意思是：每一个区块的背后，都凝结了大量的能量消耗。如果有人想篡改一个已经被写入的区块——比如把一笔已经确认的交易删掉，从而实现双花——他不仅需要重做这个区块的全部计算，还需要重做这个区块之后所有区块的计算。随着时间推移，篡改的代价呈指数级增长，很快就变得在经济上完全不可行。

这就是为什么比特币的交易一旦被确认，就几乎不可逆。不是因为有人规定不许逆转，而是因为逆转的代价高到没有人愿意支付。

更妙的是，这个机制让"自私"和"诚实"指向同一个方向。矿工不需要是好人。他们完全可以是纯粹的逐利者。但在这个竞争机制下，诚实地按规则打包交易就是利润最大化的策略。任何试图作弊的行为——比如故意篡改交易顺序——都会增加自己挖出的区块被网络拒绝的风险，白白浪费已经投入的电力成本。当理性的自利计算指向诚实行为时，系统的安全性就不再依赖于人性的善——而是依赖于人性的贪。后者是一个远比前者更可靠的假设。
...
真实的、外部的、可独立验证的。能量消耗——也就是工作量证明——是目前已知的唯一同时满足这三个条件的机制。
...
比特币的稀缺是真实的。2100万枚的总量上限不是某个公司的承诺，而是通过工作量证明在物理层面被强制执行的约束。没有任何人、任何机构、任何政府可以凭空增发哪怕一个聪。这种稀缺不依赖于任何人的善意或承诺——它依赖于物理定律。
...
>
> "该网络以其非结构化的简洁性而具有强健性。节点同时工作，几乎不需要协调。它们不需要被识别身份，因为消息不会被路由到任何特定位置，只需以尽力而为的方式传递即可。节点可以随意离开和重新加入网络，接受工作量证明链作为它们离开期间所发生事件的证明。"
...
2008年，中本聪发布白皮书。2009年1月3日，比特币网络上线。在最初的几年里，只有极少数密码学爱好者和技术极客在关注这个项目。然后比特币的价格开始上涨。然后更多的人涌入。然后"区块链"这个词被从比特币中抽离出来，变成了一个独立的概念。然后数以千计的"区块链项目"涌现——它们声称自己改进了比特币，但大多数对中本聪到底解决了什么问题一无所知。

人们把比特币理解为"一种加密货币"——这是对的，但极度肤浅。就像把飞机理解为"一种交通工具"——你没说错，但你完全没有触及空气动力学的核心。

人们把区块链理解为"一种分布式数据库"——这几乎是错的。区块链不是数据库。它是一个排序机制——一个通过工作量证明让互不信任的参与者对交易顺序达成共识的系统。如果你把排序机制拿掉（比如用权益证明替换工作量证明），剩下的确实就只是一个分布式数据库——一个笨重的、低效的分布式数据库，没有任何理由不去用传统的中心化方案。

人们说"PoW太浪费能源了，我们应该用更环保的共识机制"——这就像说"建造大楼太浪费钢材了，我们应该用纸板来建"。能量消耗不是PoW的缺陷，恰恰是它的安全来源。你消耗的能量越多，篡改记录的代价越高。你"节省"下来的能量，就是你从安全性上减掉的保障。

这些误解不是无害的。它们直接导致了行业长达十五年的歧路。
